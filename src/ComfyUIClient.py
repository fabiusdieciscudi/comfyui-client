...
def patch_workflow(workflow: dict, prompt: str, resolved: dict, merger_node_id: str) -> dict:
    """
    Deep-copy the workflow template and apply two patches:
      1. Set the merged prompt text on the Prompt merger node.
      2. Overwrite text_0 on every ShowText "Tag: w1.*" node so that the
         metadata snapshot stored by SaveImageExtended reflects the actual
         runtime values rather than the pre-parse template defaults.
    """
    ...
    title_to_tag = {v: k for k, v in TAG_NODE_TITLES.items()}
    found_tags = set()
    for node in wf.values():
        title = node.get("_meta", {}).get("title", "")
        tag = title_to_tag.get(title)
        if tag is not None:
            node["inputs"]["text_0"] = resolved[tag]
            found_tags.add(tag)
        elif title == "Prompt":
            node["inputs"]["text_0"] = re.sub(r"@[^:\s]+(:[^,\s]+)*\s*[,]*", "", strip_comments(prompt))

    missing = set(TAG_NODE_TITLES.keys()) - found_tags
    if missing:
        raise RuntimeError(f"ShowText nodes not found in workflow: {sorted(missing)}")
...
def main():
    parser = argparse.ArgumentParser(description="W1 Batch Runner for ComfyUI")
    ...
    for i, combo in enumerate(combinations, 1):
        ...
        if args.scale != 1:
            ...
            print(f"Adjusting size for scale: {new_width} x {new_height} (scale = {args.scale})")
        ...
        wf = patch_workflow(workflow_template, current_prompt, resolved, merger_node_id)
