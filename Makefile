...
Z_IMAGE_T	:= --w1 --model z_image_turbo_bf16
...
unit-tests:
	source $(VENV)/bin/activate && pytest -v --cov=comfyui_client --cov-report=html:$(BUILD)/htmlcov tests/unit
...
prompt-file: output
	$(CLIENT) submit $(Z_IMAGE_T) --prompt-file "$(PROMPT_FILE)" $(OPTIONS) --output $(OUTPUT)
